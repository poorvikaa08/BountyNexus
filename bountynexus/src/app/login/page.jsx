...
export default function Login() {
    ...
    const handleSubmit = async (e) => {
        ...
        if (res.ok) {
            router.replace("/hacker-dashboard");
        }
    };
        ...
        <div className="h-screen flex justify-center items-center">
            <form onSubmit={handleSubmit} className="bg-white p-10 rounded shadow-md w-1/2">
                ...
                <div className="mb-5">
                    ...
                    <input
                        className={`shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline ${error && !isEmailValid(email) ? "border-red-500" : ""}`}
                        type="text"
                        value={email}
